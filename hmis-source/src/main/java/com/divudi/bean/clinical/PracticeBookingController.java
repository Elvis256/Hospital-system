...
//        bi.setBookingId(getBillNumberBean().gpBookingIdGenerator());
    ...
    public Speciality getSpeciality() {
        if (speciality == null && getDoctor() != null) {
            speciality = getDoctor().getSpeciality();
            System.out.println("Auto-selected speciality: " + speciality);
        }
        return speciality;
    }
    ...
    public void listCompleteAndToCompleteBillSessions() {
        System.out.println("=== listCompleteAndToCompleteBillSessions START ===");
        
        // Force refresh doctor to current logged-in user
        Doctor currentDoctor = getDoctor();
        
        System.out.println("Logged User: " + (sessionController.getLoggedUser() != null ? sessionController.getLoggedUser().getName() : "NULL"));
        System.out.println("Current Doctor: " + (currentDoctor != null ? currentDoctor.getPerson().getNameWithTitle() : "NULL"));
        System.out.println("Session Date: " + sessionDate);

        listCompletedBillSessions();
        listToCompleteBillSessions();
        
        System.out.println("Completed Sessions: " + (completedSessions != null ? completedSessions.size() : 0));
        System.out.println("To Complete Sessions: " + (toCompleteSessions != null ? toCompleteSessions.size() : 0));
        System.out.println("=== listCompleteAndToCompleteBillSessions END ===");
    }
...
//    }
    ...
    public void listCompletedBillSessions() {
        ...
        // If no doctor found for logged-in user, return empty list
        if (currentDoctor == null) {
            completedSessions = new ArrayList<>();
            System.out.println("⚠️ No doctor found for user - returning empty completed sessions");
            return;
        }
        
        String sql = "Select bs From BillSession bs where bs.retired=false and bs.sessionDate=:ssDate "
                + " and bs.patientEncounter is not null "
                + " and bs.staff=:doctor "
                + " order by bs.serialNo";
        HashMap hh = new HashMap();
        hh.put("ssDate", sessionDate);
        hh.put("doctor", currentDoctor);
    ...
    public void listToCompleteBillSessions() {
        ...
        // If no doctor found for logged-in user, return empty list
        if (currentDoctor == null) {
            toCompleteSessions = new ArrayList<>();
            System.out.println("⚠️ No doctor found for user - returning empty to-complete sessions");
            return;
        }
        
        String sql = "Select bs From BillSession bs "
                + "where bs.retired=false and bs.sessionDate=:ssDate and bs.patientEncounter is null "
                + " and bs.staff=:doctor "
                + "order by bs.serialNo";
        HashMap hh = new HashMap();
        hh.put("ssDate", sessionDate);
        ...
        System.out.println("=== listToCompleteBillSessions details ===");
        System.out.println("Filtering for doctor: " + currentDoctor.getPerson().getNameWithTitle());
        System.out.println("Found " + (toCompleteSessions != null ? toCompleteSessions.size() : 0) + " sessions");
    }
    ...
    public ServiceSession getSelectedServiceSession() {
        ...
        System.out.println("=== getSelectedServiceSession ===");
        System.out.println("Looking for ServiceSession with doctor=" + doctor + ", sessionDate=" + sessionDate);
        selectedServiceSession = getServiceSessionFacade().findFirstByJpql(sql, m, TemporalType.DATE);
        System.out.println("Found selectedServiceSession = " + selectedServiceSession);
        ...
        if (selectedServiceSession == null) {
            ...
            System.out.println("Created new selectedServiceSession = " + selectedServiceSession);
    ...
    public Doctor getDoctor() {
        // ALWAYS validate on every call - critical for multi-user environment
        if (sessionController != null && sessionController.getLoggedUser() != null) {
            Staff loggedStaff = sessionController.getLoggedUser().getStaff();
            
            // ALWAYS refresh if logged staff is a doctor and different from cached value
            if (loggedStaff instanceof Doctor) {
                Doctor loggedDoctor = (Doctor) loggedStaff;
                // Always update to logged-in doctor
                if (doctor == null || !loggedDoctor.getId().equals(doctor.getId())) {
                    doctor = loggedDoctor;
                    System.out.println("✓ Auto-selected doctor: " + doctor.getPerson().getNameWithTitle());
                }
            } else if (loggedStaff != null) {
                // Staff exists but might not be instance of Doctor - try to find Doctor record
                if (doctor == null || !loggedStaff.getId().equals(doctor.getId())) {
                    String jpql = "SELECT d FROM Doctor d WHERE d.id = :staffId";
                    Map<String, Object> params = new HashMap<>();
                    params.put("staffId", loggedStaff.getId());
                    List<Doctor> doctors = doctorFacade.findByJpql(jpql, params);
                    if (doctors != null && !doctors.isEmpty()) {
                        doctor = doctors.get(0);
                        System.out.println("✓ Found doctor by ID: " + doctor.getPerson().getNameWithTitle());
                    } else {
                        // Try by code as fallback
                        jpql = "SELECT d FROM Doctor d WHERE d.code = :code";
                        params = new HashMap<>();
                        params.put("code", loggedStaff.getCode());
                        doctors = doctorFacade.findByJpql(jpql, params);
                        if (doctors != null && !doctors.isEmpty()) {
                            doctor = doctors.get(0);
                            System.out.println("✓ Found doctor by code: " + doctor.getPerson().getNameWithTitle());
                        } else {
                            // Staff exists but no doctor record found - clear cached doctor
                            doctor = null;
                            System.out.println("⚠️ Staff exists but is not a doctor - clearing cached doctor");
                        }
                    }
                }
            } else {
                // No staff record at all - clear cached doctor
                doctor = null;
                System.out.println("⚠️ User has no staff record - clearing cached doctor");
            }
        }
        return doctor;
    }
