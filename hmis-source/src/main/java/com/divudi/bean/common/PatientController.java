...
//        p.getPerson().setEditer(getSessionController().getLoggedUser());
    ...
    public String getCountPatientCode(String s) {
        // Handle null or empty prefix - use institution code
        if (s == null || s.trim().isEmpty()) {
            if (sessionController.getInstitution() != null && sessionController.getInstitution().getInstitutionCode() != null) {
                s = sessionController.getInstitution().getInstitutionCode();
            } else {
                s = "PT"; // Default prefix if no institution code
            }
        }
        ...
        if (p != null) {
            ...
            // Extract numeric part from code (works with any prefix length)
            String numericPart = str.replaceAll("[^0-9]", "");
            try {
                long l = Long.parseLong(numericPart);
                l++;
                st += s;
                st += df.format(l);
                return st;
