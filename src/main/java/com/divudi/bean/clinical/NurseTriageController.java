...
public class NurseTriageController implements Serializable {
    ...
    public void loadQueuedPatients() {
        ...
        // FIRST: Check ALL OPD encounters regardless of date
        String debugJpql = "SELECT e FROM PatientEncounter e WHERE e.retired = false AND e.encounterType = :encType ORDER BY e.createdAt DESC";
        Map<String, Object> debugParams = new HashMap<>();
        debugParams.put("encType", com.divudi.core.data.EncounterType.Opd);
        List<PatientEncounter> allOpdEncounters = patientEncounterFacade.findByJpql(debugJpql, debugParams);
        System.out.println("DEBUG: Total OPD encounters in DB: " + (allOpdEncounters != null ? allOpdEncounters.size() : 0));
        if (allOpdEncounters != null && !allOpdEncounters.isEmpty()) {
            System.out.println("DEBUG: Recent OPD encounters:");
            for (int i = 0; i < Math.min(5, allOpdEncounters.size()); i++) {
                PatientEncounter e = allOpdEncounters.get(i);
                System.out.println("  - BHT: " + e.getBhtNo() + ", CreatedAt: " + e.getCreatedAt() + ", Type: " + e.getEncounterType() + ", Retired: " + e.isRetired());
            }
        }

        // Show ALL OPD patients in queue
        ...
        System.out.println("EncounterType: " + com.divudi.core.data.EncounterType.Opd);

        queuedPatients = patientEncounterFacade.findByJpql(jpql, params, TemporalType.TIMESTAMP);
        
        System.out.println("Found " + (queuedPatients != null ? queuedPatients.size() : 0) + " patients in queue");
        if (queuedPatients != null && !queuedPatients.isEmpty()) {
            for (PatientEncounter enc : queuedPatients) {
                System.out.println("  - BHT: " + enc.getBhtNo() + ", Patient: " + enc.getPatient().getPerson().getName() + ", Type: " + enc.getEncounterType());
            }
        }
