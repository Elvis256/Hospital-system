...
public class DiagnosisController implements Serializable {
    ...
    public List<ClinicalEntity> completeDiagnosis(String qry) {
        ...
        String sql;
        sql = "select c from ClinicalEntity c where c.retired=false and (c.name like :n or c.code like :n) and c.symanticType=:t and (c.codeSystem='ICD-10' or c.codeSystem='ICD10') order by c.name";
        c = getFacade().findByJpql(sql, m, 20);
        if (c == null) {
            c = new ArrayList<>();
        }
        return c;
