...
public class PatientEncounterController implements Serializable {
    ...
    // Lab test bill token properties
    private String generatedBillToken;
    private PreBill generatedPreBill;
    private boolean showBillTokenDialog = false;
    ...
    public String navigateToOpdVisit() {
...
////        setCurrent(getFacade().find(current.getId()));
    ...
    public void fillMedicines(Patient patient) {
    ...
    public String navigateToOldOpdVisitFromSearch() {
        ...
        opdVisit.setEncounterType(com.divudi.core.data.EncounterType.Opd);
    ...
    public String generateDocumentFromTemplate(DocumentTemplate t, PatientEncounter e) {
        ...
        for (ClinicalFindingValue rx : getPatientMedicines()) {
            if (rx != null && rx.getPrescription() != null) {
                ...
                String frequencyUnit = rx.getPrescription().getFrequencyUnit() != null ? rx.getPrescription().getFrequencyUnit().getName() : "";
                ...
                routineMedicinesAsString += medicineName + " " + dose + " " + doseUnit;
                if (!frequencyUnit.trim().equals("")) {
                    routineMedicinesAsString += " - " + frequencyUnit;
                }
                if (!duration.trim().equals("")) {
                    routineMedicinesAsString += " - " + duration;
                }
                if (!durationUnit.trim().equals("")) {
                    routineMedicinesAsString += " - " + durationUnit;
                }
                routineMedicinesAsString += "<br/>";
            }
        }
...
//        getPharmacySaleController().getBill().setPatient(current.getPatient());
    ...
    /**
     * METHOD A: Order Lab Tests WITHOUT Cash (Direct to Lab)
     * Tests appear immediately in lab queue
     * No payment required yet
     */
    public void orderLabTestsWithoutCash() {
        if (current == null || encounterInvestigations == null || encounterInvestigations.isEmpty()) {
            JsfUtil.addErrorMessage("No investigations to send");
            return;
        }
    ...
    /**
     * METHOD A: Order Lab Tests Requiring Payment First
     * Creates PreBill token with barcode for scanning at cashier
     * Patient pays at cashier, then tests go to lab
     */
    public String orderLabTestsRequiringPaymentFirst() {
        System.out.println("=== orderLabTestsRequiringPaymentFirst START ===");
        
        if (current == null) {
            JsfUtil.addErrorMessage("No visit selected");
            return "";
        }
        
        if (encounterInvestigations == null || encounterInvestigations.isEmpty()) {
            JsfUtil.addErrorMessage("No tests ordered. Please add tests first.");
            return "";
        }
        
        try {
            // Create PreBill (not regular Bill) for token-based payment
            PreBill preBill = new PreBill();
            preBill.setBillType(BillType.OpdBill);
            preBill.setBillTypeAtomic(BillTypeAtomic.OPD_BATCH_BILL_TO_COLLECT_PAYMENT_AT_CASHIER);
            preBill.setPatient(current.getPatient());
            preBill.setPatientEncounter(current);
            ...
            preBill.setToDepartment(sessionController.getDepartment());
            preBill.setToInstitution(sessionController.getInstitution());
            preBill.setBillDate(new Date());
            preBill.setBillTime(new Date());
            preBill.setCreatedAt(new Date());
            preBill.setCreater(sessionController.getLoggedUser());
            preBill.setComments("LAB TESTS PRE-BILL - Patient must pay at cashier before lab processing");
            
            // First create the preBill to get ID
            billFacade.create(preBill);
            
            // Generate bill numbers using the pharmacy pattern
            String insId = billNumberGenerator.institutionBillNumberGenerator(
                preBill.getInstitution(), 
                preBill.getBillType(), 
                BillClassType.PreBill, 
                BillNumberSuffix.NONE
            );
            preBill.setInsId(insId);
            
            String deptId = billNumberGenerator.departmentBillNumberGenerator(
                preBill.getDepartment(), 
                preBill.getBillType(), 
                BillClassType.PreBill, 
                BillNumberSuffix.NONE
            );
            preBill.setDeptId(deptId);
            
            // Add investigation items to bill (allow zero prices - cashier will update)
            double total = 0.0;
            ...
            for (ClinicalFindingValue cfv : encounterInvestigations) {
                if (cfv.getItemValue() != null) {
                    BillItem bi = new BillItem();
                    bi.setBill(preBill);
                    bi.setItem(cfv.getItemValue());
                    bi.setQty(1.0);
                    
                    Double rate = cfv.getItemValue().getTotal();
                    ...
                    // Mark test as requiring payment
                    cfv.setRequirePaymentBeforeLab(true);
                    clinicalFindingValueFacade.edit(cfv);
                }
            }
            ...
            preBill.setDiscount(0.0);
            preBill.setBalance(total);
            ...
            preBill.setBillItems(billItems);
            billFacade.edit(preBill);
            
            // Store for print page
            this.generatedBillToken = insId;
            this.generatedPreBill = preBill;
            
            // Success message
            JsfUtil.addSuccessMessage("Bill Token Generated Successfully: " + insId);
            
            System.out.println("=== orderLabTestsRequiringPaymentFirst SUCCESS ===");
            System.out.println("Bill Token: " + insId);
            
            // Navigate to print page
            return "/lab/lab_bill_token_print?faces-redirect=true";
            
        } catch (Exception e) {
            System.out.println("=== orderLabTestsRequiringPaymentFirst ERROR ===");
            System.out.println("Error: " + e.getMessage());
            e.printStackTrace();
            JsfUtil.addErrorMessage("Error generating bill token: " + e.getMessage());
            return "";
        }
    }

    /**
     * METHOD B: Order Lab Tests WITH Payment First (OLD METHOD - KEPT FOR COMPATIBILITY)
     * Creates a bill TOKEN with barcode
     * Patient takes token to cashier
     * Cashier scans barcode, items auto-populate, patient pays
     * After payment, tests go to lab
     */
    public String orderLabTestsWithPayment() {
        if (current == null) {
            JsfUtil.addErrorMessage("No visit selected");
            return "";
        }
        ...
        try {
            // Create a new bill for lab tests
            Bill bill = new Bill();
            bill.setBillType(BillType.OpdBill);
            bill.setPatient(current.getPatient());
            bill.setPatientEncounter(current);
            ...
            bill.setCreatedAt(new Date());
            bill.setCreater(sessionController.getLoggedUser());
            bill.setComments("LAB TESTS - Payment Required - Scan barcode at cashier");
            ...
            // Add investigation items to bill
            double total = 0.0;
            List<BillItem> billItems = new ArrayList<>();
            ...
            // Now set bill on items and save them
            for (BillItem bi : billItems) {
                bi.setBill(bill);
                billItemFacade.create(bi);
            }
            ...
            // Print bill token with barcode
            JsfUtil.addSuccessMessage("Bill Token Generated: " + billNumber + 
                ". Patient should take this token to cashier for payment. " +
                "Tests will go to lab AFTER payment.");
            
            // Set bill for display/printing
            getBillController().setBill(bill);
            getBillController().setPrintPreview(true);
            
            // Navigate to bill print/display page
    ...
    /**
     * View Lab Results
     * Navigate to results viewing page
     */
    public String viewLabResults() {
        if (current == null) {
            JsfUtil.addErrorMessage("No visit selected");
            return "";
        }
        ...
        return "/lab/lab_results_for_patient?faces-redirect=true";
    }

    /**
     * Save Bill Items (After Reviewing Results)
     * Pushes all services including medications to billing
     * Biller receives auto-populated items
     */
    public String saveBillItems() {
        if (current == null) {
            JsfUtil.addErrorMessage("No visit selected");
            return "";
        }
        
        // Navigate to billing with ALL encounter items
        getBillController().prepareNewBill();
        getBillController().setSearchedPatient(current.getPatient());
        getBillController().setFromOpdEncounter(true);
        getBillController().setOpdEncounterComments("COMPLETE VISIT BILL: " + (current.getComments() != null ? current.getComments() : ""));
        getBillController().setPatientSearchTab(1);
        getBillController().setPatientTabId("tabSearchPt");
        getBillController().setReferredBy(doctor);
        
        // Set patient encounter on bill
        if (getBillController().getBill() != null) {
            getBillController().getBill().setPatientEncounter(current);
            getBillController().getBill().setComments("AUTO-POPULATED - Complete OPD Visit");
        }
        
        int itemCount = 0;
        if (encounterInvestigations != null) itemCount += encounterInvestigations.size();
        if (encounterMedicines != null) itemCount += encounterMedicines.size();
        
        JsfUtil.addSuccessMessage("Saved " + itemCount + " item(s) to billing. Items will auto-populate.");
        return "/opd/opd_bill";
    }
    ...
    public String generateBillForPatient() {
        return saveBillItems();
    }

    /**
     * Send to Billing Desk
     * For insurance, credit, or membership scheme billing
     */
    public String sendToBillingDesk() {
        if (current == null) {
            JsfUtil.addErrorMessage("No visit selected");
            return "";
        }
        
        // Similar to generate bill but marked for billing desk
        getBillController().prepareNewBill();
        getBillController().setSearchedPatient(current.getPatient());
        getBillController().setFromOpdEncounter(true);
        getBillController().setOpdEncounterComments("FROM OPD - BILLING DESK: " + (current.getComments() != null ? current.getComments() : ""));
        getBillController().setPatientSearchTab(1);
        getBillController().setPatientTabId("tabSearchPt");
        getBillController().setReferredBy(doctor);
        
        // Mark this bill for billing desk processing (insurance/credit)
        if (getBillController().getBill() != null) {
            getBillController().getBill().setCreditBill(true);
            getBillController().getBill().setPatientEncounter(current);
            getBillController().getBill().setComments("BILLING DESK - Insurance/Credit - Auto-populated");
        }
        ...
        JsfUtil.addSuccessMessage("Sent " + itemCount + " items to billing desk. Items linked to bill via encounter.");
        return "/opd/opd_bill";
    }
    ...
    public List<ClinicalFindingValue> fillEncounterDiagnoses(PatientEncounter encounter) {
        List<ClinicalFindingValueType> clinicalFindingValueTypes = new ArrayList<>();
    ...
    // Getters and setters for bill token dialog
    public String getGeneratedBillToken() {
        return generatedBillToken;
    }

    public void setGeneratedBillToken(String generatedBillToken) {
        this.generatedBillToken = generatedBillToken;
    }

    public PreBill getGeneratedPreBill() {
        return generatedPreBill;
    }

    public void setGeneratedPreBill(PreBill generatedPreBill) {
        this.generatedPreBill = generatedPreBill;
    }

    public boolean isShowBillTokenDialog() {
        return showBillTokenDialog;
    }

    public void setShowBillTokenDialog(boolean showBillTokenDialog) {
        this.showBillTokenDialog = showBillTokenDialog;
    }
