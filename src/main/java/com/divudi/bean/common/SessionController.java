...
//    }
    ...
    // HYBRID ACCESS MODEL: Role-based access + Staff-based HR services
    // Role grants module access (clinical, pharmacy, etc.)
    // Staff profile adds HR services (payslip, leave, etc.)
    ...
    public boolean hasRole(String roleName) {
        System.out.println("DEBUG hasRole(): Checking role '" + roleName + "' for user: " + 
                          (loggedUser != null ? loggedUser.getName() : "NULL"));
        
        if (loggedUser == null || roleName == null) {
            System.out.println("DEBUG hasRole(): User or roleName is NULL - returning false");
            return false;
        }
        
        System.out.println("DEBUG hasRole(): loggedUser.getRole() = " + loggedUser.getRole());
        
        // Check single role field
        if (loggedUser.getRole() != null && loggedUser.getRole().getName() != null) {
            String userRoleName = loggedUser.getRole().getName();
            System.out.println("DEBUG hasRole(): User role name = '" + userRoleName + "', comparing with '" + roleName + "'");
            if (userRoleName.equalsIgnoreCase(roleName)) {
                System.out.println("DEBUG hasRole(): MATCH! Returning true");
                return true;
            } else {
                System.out.println("DEBUG hasRole(): NO MATCH! '" + userRoleName + "' != '" + roleName + "'");
            }
        } else {
            System.out.println("DEBUG hasRole(): Role or role name is NULL");
        }
    ...
    /**
     * Check if user is a nurse (for nursing access)
    ...
    /**
     * Check if user is a pharmacist (for pharmacy access)
     * @return true if user has Pharmacist role
     */
    ...
    /**
     * Check if user is a lab technician (for lab access)
     * @return true if user has Lab Technician role
     */
    public boolean isLabTechnician() {
        return hasRole("Lab Technician") || hasRole("LAB TECHNICIAN");
    }
    
    /**
     * Check if user is an employee (has staff profile)
     * Used to show/hide HR self-service features
     * @return true if user has staff profile
     */
    public boolean isEmployee() {
        return loggedUser != null && loggedUser.getStaff() != null;
    }
