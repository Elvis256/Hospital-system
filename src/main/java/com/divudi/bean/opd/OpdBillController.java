...
//        setReferredByInstitution(null);
    ...
    /**
     * Auto-populate bill items from patient encounter
     * Called when bill has a linked encounter with ordered items
     */
    public void autoPopulateFromEncounter() {
        if (bill == null || bill.getPatientEncounter() == null) {
            return;
        }
        
        // Check if bill already has items (avoid duplicate population)
        if (bill.getBillItems() != null && !bill.getBillItems().isEmpty()) {
            return;
        }
        
        PatientEncounter encounter = bill.getPatientEncounter();
        
        try {
            // Load investigations from encounter
            String jpql = "SELECT cfv FROM ClinicalFindingValue cfv "
                        + "WHERE cfv.retired = false "
                        + "AND cfv.patientEncounter = :encounter "
                        + "AND cfv.clinicalFindingValueType = :type "
                        + "AND cfv.itemValue IS NOT NULL";
            
            Map<String, Object> params = new HashMap<>();
            params.put("encounter", encounter);
            params.put("type", ClinicalFindingValueType.VisitInvestigation);
            
            List<ClinicalFindingValue> investigations = clinicalFindingValueFacade.findByJpql(jpql, params);
            
            int itemsAdded = 0;
            List<BillItem> newBillItems = new ArrayList<>();
            
            // Convert investigations to bill items
            for (ClinicalFindingValue cfv : investigations) {
            ...
            // Load medicines from encounter
            params.put("type", ClinicalFindingValueType.VisitMedicine);
            List<ClinicalFindingValue> medicines = clinicalFindingValueFacade.findByJpql(jpql, params);
            
            // Convert medicines to bill items
            for (ClinicalFindingValue cfv : medicines) {
                if (cfv.getItemValue() != null) {
                    BillItem bi = new BillItem();
                    bi.setBill(bill);
                    bi.setItem(cfv.getItemValue());
                    
                    // Get quantity from cfv if available
                    Double qty = 1.0;
                    double qtyValue = cfv.getDoubleValue();
                    if (qtyValue > 0) {
                        qty = qtyValue;
                    }
                    bi.setQty(qty);
                    ...
                    bi.setRate(rate);
                    bi.setGrossValue(rate * qty);
                    bi.setNetValue(rate * qty);
                    bi.setDiscount(0.0);
                    bi.setCreatedAt(new Date());
                    bi.setCreater(sessionController.getLoggedUser());
            ...
            if (itemsAdded > 0) {
                ...
                bill.getBillItems().addAll(newBillItems);
                
                // Calculate bill totals
                calTotals();
                JsfUtil.addSuccessMessage("Auto-populated " + itemsAdded + " item(s) from OPD visit.");
            }
            
        } catch (Exception e) {
            System.err.println("Error auto-populating from encounter: " + e.getMessage());
            e.printStackTrace();
        }
    }
    
    public String navigateToNewOpdOrder() {
        // OPD Ordering for doctors/nurses - no shift requirement
        patientController.setQuickSearchPhoneNumber(null);
        clearBillItemValues();
        clearBillValues();
        paymentMethodData = null;
        paymentScheme = null;
        paymentMethod = PaymentMethod.Credit; // Default to Credit for orders
        patientEncounter = null;
        collectingCentreBillController.setCollectingCentre(null);
        return "/opd/opd_order?faces-redirect=true";
    }
