...
//                continue;
    ...
    private void saveSaleBillItems(List<BillItem> list) {
        for (BillItem tbi : list) {
            ...
            // FIX: Create brand new Prescription object to avoid managed entity duplicate key errors
            if (tbi.getPrescription() != null) {
                Prescription oldPrescription = tbi.getPrescription();
                Prescription newPrescription = new Prescription();
                
                // Copy all data from old to new prescription
                newPrescription.setComment(oldPrescription.getComment());
                newPrescription.setPatient(patient);
                newPrescription.setCreatedAt(new Date());
                newPrescription.setCreater(sessionController.getWebUser());
                newPrescription.setInstitution(sessionController.getInstitution());
                newPrescription.setDepartment(sessionController.getDepartment());
                newPrescription.setItem(oldPrescription.getItem());
                newPrescription.setDose(oldPrescription.getDose());
                newPrescription.setDoseUnit(oldPrescription.getDoseUnit());
                newPrescription.setDuration(oldPrescription.getDuration());
                newPrescription.setDurationUnit(oldPrescription.getDurationUnit());
                newPrescription.setFrequencyUnit(oldPrescription.getFrequencyUnit());
                newPrescription.setIssueUnit(oldPrescription.getIssueUnit());
                newPrescription.setIssue(oldPrescription.getIssue());
                
                // Create NEW object (JPA assigns fresh ID from sequence)
                prescriptionFacade.create(newPrescription);
                
                // Use the new prescription for both bill items
                newBil.setPrescription(newPrescription);
                tbi.setPrescription(newPrescription);
            }
